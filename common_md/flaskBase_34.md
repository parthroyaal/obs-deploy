...
# data_dir =  "C:/Users/acer/Documents/y2025/data/"
...
def get_historical_data():
    ...
    try:
        ...
        df['timestamp'] = pd.to_datetime(df['timestamp'].str.strip(), utc=True, format='mixed')
        df['timestamp'] = df['timestamp'].astype(int) // 10**6  # Convert to miliseconds
...
const chart = LightweightCharts.createChart(document.getElementById('chart'), {
    ...
    height: window.innerHeight,
       grid: {
        ...
        crosshair: {
        mode: LightweightCharts.CrosshairMode.Normal, // This disables the "magnet" behavior.
    },
    priceScale: { borderColor: '#cccccc' },
    timeScale: { 
        borderColor: '#cccccc', 
    ...
    localization: {
        timeFormatter: (time) => {
            const date = new Date(time * 1000);
            // Add 5 hours and 30 minutes for IST (same as for axis)
            // date.setTime(date.getTime() + (5.5 * 60 * 60 * 1000));
            // return date.toLocaleTimeString('en-IN');
            return date.toLocaleDateString('en-IN') + ' ' + date.toLocaleTimeString('en-IN');
