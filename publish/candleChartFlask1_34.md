...
- Initial ticks (0-599) start at 100.0 and grow by 0.1% each

- WebSocket ticks (600-1199) continue from 100.0 * (1.001^600)

- No price reset between historical and real-time data

The previous implementation was resetting to 100.0 for both segments because the WebSocket data generation wasn't accounting for the initial 600 ticks' growth.
...
def generate_mock_ticks(num_rows, start_index=0):
    ...
    # Start from previous value when continuing
    if start_index > 0:
        # Calculate the value after previous ticks
        tick_value = 100.0 * (1.001 ** start_index)
...
def push_realtime_ticks(ws):
    # Start from index 600 (after initial 600 ticks)
