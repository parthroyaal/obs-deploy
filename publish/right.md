...
<body>
  ...
  <script>
     ...
     * Groups ticks by their Unix timestamp. For each group, the first tick is
     * used as the open, the last tick as the close, and high/low are calculated.
     */
    function aggregateToCandlestick(data) {
      const candles = {};
      data.forEach((item) => {
        const t = item.time; // Unix timestamp in seconds
        if (!candles[t]) {
          // initialize a new candle for this timestamp
          candles[t] = {
            time: t,
            open: item.close,
            high: item.close,
          ...
          candles[t].high = Math.max(candles[t].high, item.close);
          candles[t].low = Math.min(candles[t].low, item.close);
          candles[t].close = item.close; // update closing price to the latest tick
        }
      });
      // Convert the candle object into an array and sort by time
